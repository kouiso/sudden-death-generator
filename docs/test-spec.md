...
## E. 探索的 dogfooding で発見した重大な表示崩れ（自動テストが見逃していた不具合）

D の時点までの自動テストは全て `textContent`（DOM 上の文字列）だけを検証しており、実際に
画面へどう描画されるかは確認していなかった。ユーザーからの指摘を受け、固定シナリオではなく
自由な入力（絵文字、長文など）で実際に触ってスクリーンショットを目視した結果、実害を発見・修正した。

### E1. 非日本語文字の幅誤判定（Codex bot 指摘、実機で再現確認 → 製品スコープ判断で対応方針を revert）

`charWidth` がラテン文字拡張・ギリシャ文字・キリル文字・ベトナム語の合成済み声調記号付き文字・
スマート引用符/ダッシュ/三点リーダーを「未知の文字」として全角(2)扱いしていたため、
`café au lait` や `Привет` `Γειά` `Tiếng Việt` `Wait—what…` のような入力で右側にだけ
余分な空白が入り左右非対称になることを、その都度実機のスクリーンショットで確認・修正していた。

その後、本ツールは日本語入力のみを前提とし海外での利用シーンを想定しない、という製品スコープの
判断により、上記の非日本語文字向け幅判定コード（`NARROW_RANGES`）は撤去した。`charWidth` は
ASCII・半角カタカナ以外を一律全角として扱うシンプルな実装に戻している（`src/core/width.ts`）。
この判断以降、同種の「海外文字の幅がズレる」という指摘は本ツールの対象外として扱う。

### E2. 四角形・短冊の枠線がほぼ画面から消える（探索的 dogfooding で発見、自動テストは検出不能だった）

四角形・短冊を選んで実際に画面を見ると、枠の罫線行がほとんど見えず、本体行だけが表示される
現象を発見した。原因を `fonttools` でサブセットフォント (`sd-symbols.woff2`) のメトリクスを
直接検査して特定: `hhea.ascent(1160) + |descent|(288)` が `unitsPerEm(1000)` の **1.448倍**
あり、CSS の `line-height: 1.2` では隣接行のグリフが重なって視覚的に消えていた
（`.preview-pre` の全行が sd-symbols フォントのグリフだけで構成される行ほど症状が顕著）。

単独の HTML で `line-height` を 1.2 / 1.4 / 1.45 / 1.5 / 1.6 / 2 / 3 / normal と変えて
再現条件を切り分け、理論値(1.448)以上で解消することを確認。`src/styles/global.css` の
`.preview-pre` の `line-height` を `1.2` → `1.5` に変更し、四角形・短冊・複数行通常形・
縦書き・ストレスの全形状で枠線が正しく表示されることをスクリーンショットで再確認した。

**教訓**: `pre.textContent` の一致だけを検証する自動テストは、フォントメトリクスに起因する
視覚的な描画崩れを検出できない。今後、フォント絡みの変更をする際は必ず実際のスクリーンショットを
目視確認する（撮影しただけで終わらせない）。
