...
自由な入力（`café au lait` のようなラテン文字拡張、絵文字、長文など）で実際に触って
スクリーンショットを目視した結果、2件の実害を発見・修正した。

### E1. アクセント付きラテン文字（é ñ ø ß œ 等）の幅誤判定（Codex bot 指摘、実機で再現確認）

`charWidth` が Latin-1 Supplement / Latin Extended-A/B を「未知の文字」として全角(2)扱い
していたため、`café au lait très bien` のような入力で右側にだけ余分な空白が入り、左右非対称に
なることを実機のスクリーンショットで確認した。`src/core/width.ts` に半角と判定する範囲
(U+00A0–U+024F) を追加して修正し、修正前後のスクリーンショットで対称になったことを確認した。
