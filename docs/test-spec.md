...
| B10 | X シェア | 「X でシェア」リンク | `href` が `https://twitter.com/intent/tweet?text=<encodeURIComponent(現在の出力)>` になっている |
| B11 | LINE 送信 | 「LINE で送信」リンク | `href` が `https://line.me/R/share?text=<encodeURIComponent(現在の出力)>` になっている |
...
| B16 | 共有ボタンの折り返し | ビューポート幅 320〜428px（iPhone SE〜Pro Max 相当）でコピー | 「𝕏 でシェア」「LINE で送信」のラベルが1行のまま折り返さない（各 `.share-button` の `scrollWidth` が `clientWidth` を超えない） |
| B17 | コピー通知トーストの非重なり | 同幅域でコピーボタンを押す | `.toast` と `.share-bar` の矩形が重ならない（`getBoundingClientRect()` で判定） |
...
### E3. 実機幅で共有ボタンの文字折り返し + コピー通知トーストの重なり（探索的 dogfooding で発見）

固定シナリオではなく、実際に ITO ゲームで負けた場面を想定して入力・操作した dogfooding で発見。

- **共有ボタンの折り返し**: `.share-bar` は `max-width: 380px` を境に1列へ切り替えていたが、
  iPhone 12/13/14（390px）・Pixel 7（412px）・iPhone Pro Max（428px）など主要スマートフォンは
  いずれも380pxを超えるため3列のまま残り、「𝕏 でシェア」「LINE で送信」の文字が枠内で折り返して
  読みにくくなっていた。420pxで折り返し発生・440pxで解消することを実測し、実機幅に十分な余裕を
  持たせて閾値を`480px`に引き上げた。
- **トーストの重なり**: コピー成功時に表示される `.toast` は `position: fixed; bottom: 24px` で
  画面下端に固定していたが、共有ボタン列も画面下部にあるため、画面高さやスクロール位置によっては
  ボタンのラベルに重なって隠れることを実機幅×高さの組み合わせ（iPhone SE 320×568 〜 iPhone Pro Max
  428×926）で確認した。固定オフセット値（`bottom: 180px`等）で個別に回避を試みたが、画面高さが
  異なると同じ値では再現性が無いことも実測で判明したため、根本的に `position: fixed` の
  オーバーレイ方式をやめ、`ShareBar` 直下のドキュメントフロー内に配置する方式に変更した。
  これにより画面サイズ・スクロール位置に関わらず重ならないことが構造的に保証される。

**教訓**: 既存の B13（モバイル幅テスト）は `body.scrollWidth` による横スクロールの有無しか
検証しておらず、要素内でのテキスト折り返しや固定要素同士の重なりは検出できなかった。
横スクロールが無い＝モバイルで正しく表示される、ではない。

### E4. 元サイト3つとの機能網羅比較（ユーザー指摘を受けて実ソースを取得し比較、2026-08-06）

「元サイトにあった機能は全部網羅されてるか」という指摘を受け、Phase 0 時点の WebFetch
テキスト調査（未検証のまま「確認済み」と書いていた）をやり直し、`curl` で実ソース
（HTML/JS）を取得して直接比較した。

- **接続できたサイト**: echo-sd（osstech）、ichitaso — HTML/JS を全文取得できた
- **接続できなかったサイト**: sacnoha — `curl` が毎回 `Connection reset by peer`。
  この環境からは検証不能なまま（「3サイト全部ブロック」という当初の記述は不正確だった。
  osstech と ichitaso は普通に到達できた）

実ソース比較で見つかった機能差分と対応:

| # | 差分 | 対応 |
|---|---|---|
| 1 | 縦書きの記号カバー範囲が echo-sd の `VC_MAP` より狭かった（：！？…や各種括弧が非対応） | Unicode の Vertical Forms / CJK Compatibility Forms の正式な提示形を `unicodedata` で個別検証した上で `vertical.ts` に追加（echo-sd の記号選定自体は転記せず独自に Unicode 規格から導出） |
| 2 | 短冊の上枠に echo-sd にある紐穴マーク「-┷-」が無かった | `render.ts` に自前実装で追加。枠全体の幅を変えずに中央へ挿入する |
| 3 | ストレス形状の内部構造が echo-sd と別物だった（旧: 全行を↘だけで繋いで最終行を複製。本家: 固定4段のジグザグ(↘↙交互)+5行目以降を別枠へ） | `render.ts` の `renderStress` を本家と同じ構造に作り直し（コードは転記せず、挙動だけを自前実装） |
| 4 | パーマリンク（URL一つで入力・オプション状態を復元）が無かった | `src/core/permalink.ts` を新規実装。`history.replaceState` でURLを常時同期し、「リンクをコピー」ボタンを追加 |
| 5 | アクセスキー（V/P/N/4/T/S のキーボードショートカット）が無かった | 各チェックボックス・スタイル選択ボタンに `accessKey` 属性を追加 |
| 6 | ドキュメントの事実誤認：「元サイト3つはいずれも生成ボタン必須」と書いていたが、osstech の `echo-sd.js` を読んだら実際は `input`/`change` イベントで即再描画するリアルタイムプレビュー方式だった（生成ボタン必須は ichitaso のみ） | `docs/dogfooding-report.md` の記述を訂正 |

**教訓**: 「元サイトを参考にした」機能比較は、一度 WebFetch でテキストを読んだだけで
「確認済み」と書いてはいけない。実際に到達できるならソースを取得して直接比較する。
到達できないサイト（sacnoha）は「検証不能」と正直に書き、機能比較済みであるかのように
装らない。
