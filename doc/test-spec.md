...
再現条件を切り分け、理論値(1.448)以上で解消することを確認。`src/style/global.css` の
...
| 6 | ドキュメントの事実誤認：「元サイト3つはいずれも生成ボタン必須」と書いていたが、osstech の `echo-sd.js` を読んだら実際は `input`/`change` イベントで即再描画するリアルタイムプレビュー方式だった（生成ボタン必須は ichitaso のみ） | `doc/dogfooding-report.md` の記述を訂正 |
